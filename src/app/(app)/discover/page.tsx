...
export default function DiscoverPage() {
  ...
  async function handleFavorite(profileId: string): Promise<boolean> {
    try {
          ...
          <div className="flex items-center gap-2">
            <div className="flex rounded-xl overflow-hidden border border-dark-600">
              <button
                ...
                className={`p-2 transition-colors ${viewMode === 'feed' ? 'bg-gold-500/20 text-gold-400' : 'bg-dark-800 text-dark-300 hover:text-dark-50'}`}
                title="Feed TikTok"
              >
                <Play size={15} />
              </button>
              <button
                ...
                className={`p-2 transition-colors ${viewMode === 'grid' ? 'bg-gold-500/20 text-gold-400' : 'bg-dark-800 text-dark-300 hover:text-dark-50'}`}
                title="Grade"
              >
                <LayoutGrid size={15} />
              </button>
          ...
          <div className="max-w-2xl mx-auto mt-3 p-4 bg-dark-800 rounded-xl border border-dark-600 grid grid-cols-2 md:grid-cols-4 gap-3">
            <div>
              ...
              <select value={filters.state} onChange={e => setFilters(f => ({ ...f, state: e.target.value }))}
                className="w-full bg-dark-700 border border-dark-500 rounded-lg px-3 py-2 text-sm text-dark-50 focus:outline-none focus:border-gold-500/50">
            ...
            <div>
              ...
              <input value={filters.city} onChange={e => setFilters(f => ({ ...f, city: e.target.value }))}
                ...
                className="w-full bg-dark-700 border border-dark-500 rounded-lg px-3 py-2 text-sm text-dark-50 placeholder-dark-400 focus:outline-none focus:border-gold-500/50" />
            </div>
            <div>
            ...
            {limitModal === 'subscription' ? (
              <>
                ...
                <div className="flex gap-3">
                  <button onClick={() => setLimitModal(null)} className="flex-1 py-2.5 rounded-xl border border-dark-500 text-sm text-dark-300 hover:text-dark-50 transition-colors">Depois</button>
                  <Link href="/subscription" onClick={() => setLimitModal(null)} className="flex-1 bg-gold-500 hover:bg-gold-400 text-dark-50 font-bold py-2.5 rounded-xl text-sm text-center transition-colors">
                    Assinar agora
                  </Link>
                </div>
              </>
              ...
              <>
                ...
                <div className="flex gap-3">
                  <button onClick={() => setLimitModal(null)} className="flex-1 py-2.5 rounded-xl border border-dark-500 text-sm text-dark-300 hover:text-dark-50 transition-colors">Depois</button>
                  <Link href="/verification" onClick={() => setLimitModal(null)} className="flex-1 bg-gold-500 hover:bg-gold-400 text-dark-50 font-bold py-2.5 rounded-xl text-sm text-center transition-colors">
                    Verificar perfil
                  </Link>
                </div>
              </>
          ...
          <div className="glass rounded-2xl p-10 border border-gold-500/30 text-center max-w-sm mx-4 gold-glow">
            ...
            <div className="flex gap-3">
              ...
              <Link href="/matches" onClick={() => setMatchedProfile(null)}
                className="flex-1 bg-gold-500 hover:bg-gold-400 text-dark-50 px-4 py-3 rounded-xl text-sm font-bold text-center transition-colors">
                Enviar Mensagem
              </Link>
            </div>
          </div>
...
function TikTokFeed({ profiles, likedIds, onLike, onFavorite, onBlock, onReload }: {
  profiles: Profile[]
  likedIds: Set<string>
...
function TikTokCard({ profile, liked, onLike, onFavorite, onBlock, onNext, onPrev, current, total }: {
  profile: Profile
  liked: boolean
  onLike: () => void
  ...
  return (
    <div className="relative w-full h-full select-none">
      ...
      <div className="absolute top-4 left-0 right-0 px-4 flex items-center gap-2 z-10">
        ...
        <span className="text-xs text-dark-50/60 shrink-0 bg-black/30 px-2 py-0.5 rounded-full">
          {current}/{total}
        </span>
      </div>
      ...
      <div className="absolute right-4 bottom-44 flex flex-col items-center gap-5 z-20 md:bottom-36">
        <button onClick={e => { e.stopPropagation(); onBlock() }} className="flex flex-col items-center gap-1">
          ...
          <span className="text-xs text-dark-50/70">Bloquear</span>
        </button>
        ...
        >
          <div className="w-12 h-12 rounded-full bg-black/50 border border-white/20 flex items-center justify-center backdrop-blur-sm">
            <Info size={20} className="text-dark-50" />
          </div>
          <span className="text-xs text-dark-50/70">Perfil</span>
        </Link>

        <button onClick={handleSave} disabled={favoriting} className="flex flex-col items-center gap-1">
          <div className={`w-12 h-12 rounded-full border flex items-center justify-center backdrop-blur-sm transition-colors ${saved ? 'bg-black/50 border-gold-500' : 'bg-black/50 border-white/20 hover:bg-white/10'}`}>
            {favoriting
              ? <div className="w-5 h-5 border-2 border-gold-500/30 border-t-gold-500 rounded-full animate-spin" />
              : <Star size={20} className={saved ? 'fill-gold-500 text-gold-500' : 'text-dark-50'} />
            }
          </div>
          <span className="text-xs text-dark-50/70">{saved ? 'Favoritado!' : 'Favoritar'}</span>
        </button>
      </div>
      ...
      <div className="absolute right-4 top-1/2 -translate-y-1/2 hidden md:flex flex-col gap-2 z-10">
        {onPrev && (
          <button
            onClick={onPrev}
            className="w-9 h-9 rounded-full bg-black/40 border border-white/20 flex items-center justify-center text-dark-50 hover:bg-black/60 transition-colors"
          >
            <ChevronUp size={18} />
          </button>
        )}
      ...
      <div className="absolute bottom-0 left-0 right-0 px-5 pb-20 md:pb-6 z-10 pointer-events-none">
        ...
        <div className="mb-4">
          <div className="flex items-end gap-3 mb-1">
            ...
            {age && (
              <span className="text-2xl font-light text-dark-50/80 leading-none mb-0.5">{age}</span>
            )}
          </div>
          {(profile.city || profile.state) && (
            <p className="text-sm text-dark-50/60 flex items-center gap-1.5 mt-1">
              <MapPin size={13} />
              {[profile.city, profile.state].filter(Boolean).join(', ')}
            </p>
          )}
          {profile.bio && (
            <p className="text-sm text-dark-50/55 mt-1.5 line-clamp-2 max-w-[280px]">{profile.bio}</p>
          )}
          {profile.interests && profile.interests.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mt-2">
              {profile.interests.slice(0, 3).map(i => (
                <span key={i} className="px-2 py-0.5 rounded-full text-xs bg-black/40 border border-white/20 text-dark-50/70">
                  {i}
                </span>
              ))}
            </div>
