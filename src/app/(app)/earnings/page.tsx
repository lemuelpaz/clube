  ...
  if (role !== 'FEMALE') {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4 p-6 text-center">
        ...
        <Link href="/discover" className="bg-gold-500 text-dark-900 font-bold px-6 py-2.5 rounded-full text-sm">
          Voltar
        </Link>
      </div>
    )
    ...
    <div className="min-h-screen pb-24 md:pb-8">
      <div className="sticky top-0 z-30 bg-dark-800/90 backdrop-blur-lg border-b border-dark-700/60 px-6 py-4">
        <div className="max-w-2xl mx-auto flex items-center gap-3">
          <Link href="/profile" className="text-dark-300 hover:text-white transition-colors">
            <ArrowLeft size={20} />
          </Link>
      ...
      <div className="max-w-2xl mx-auto p-6 space-y-5">
        ...
        <div className="glass rounded-2xl p-6 border border-gold-500/20 gold-glow">
          ...
          <button
            ...
            className="mt-4 w-full flex items-center justify-center gap-2 py-3 rounded-xl font-bold bg-gold-500 hover:bg-gold-400 text-dark-900 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
          >
        ...
        <div className="flex rounded-xl bg-dark-700/50 border border-dark-600/60 p-1">
          {(['history', 'withdrawals'] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${tab === t ? 'bg-dark-800 text-white shadow' : 'text-dark-300 hover:text-white'}`}>
        ...
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="glass rounded-2xl border border-dark-500/60 w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-5 py-4 border-b border-dark-600/60 sticky top-0 bg-dark-800/95 backdrop-blur-lg rounded-t-2xl">
              ...
              <button onClick={() => setShowWithdraw(false)} className="text-dark-300 hover:text-white"><X size={20} /></button>
            </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder="0,00"
                        className="w-full bg-dark-700 border border-dark-500 rounded-xl pl-10 pr-4 py-2.5 text-sm text-white focus:outline-none focus:border-gold-500/40"
                      />
                    </div>
                  </div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <input value={bankForm.holderName} onChange={e => setBankForm(f => ({ ...f, holderName: e.target.value }))}
                            className="w-full bg-dark-700 border border-dark-500 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-gold-500/40" />
                        </div>
                        <div>
                      ...
                      <div>
                        ...
                        <div className="flex gap-2">
                          <select value={bankForm.pixKeyType} onChange={e => setBankForm(f => ({ ...f, pixKeyType: e.target.value as any }))}
                            className="bg-dark-700 border border-dark-500 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-gold-500/40 w-24 shrink-0">
                          ...
                          <input value={bankForm.pixKey} onChange={e => setBankForm(f => ({ ...f, pixKey: e.target.value }))}
                            ...
                            className="flex-1 bg-dark-700 border border-dark-500 rounded-xl px-3 py-2 text-sm text-white focus:outline-none focus:border-gold-500/40" />
                        </div>
                      </div>
                    </div>
                  </div>
