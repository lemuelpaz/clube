      ...
      <div className="max-w-5xl mx-auto p-6">
        {loading ? (
          ...
          <div className="text-center py-24">
            ...
            <Link href="/discover" className="inline-block mt-4 bg-gold-500 hover:bg-gold-400 text-dark-900 font-bold px-6 py-3 rounded-full text-sm transition-colors">
              Descobrir Perfis
            </Link>
          </div>
        ) : (
                ...
                <div key={p.id} className="group relative rounded-2xl overflow-hidden bg-dark-800 border border-dark-600 hover:border-gold-500/30 transition-all card-hover">
                  <Link href={`/profile/${p.id}`}>
                    <div className="aspect-[3/4] relative overflow-hidden bg-dark-700">
                      ...
                      {p.verified && (
                        <div className="absolute top-2 right-2 bg-gold-500/90 rounded-full p-1"><Shield size={12} className="text-dark-900" /></div>
                      )}
