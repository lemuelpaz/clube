...
import { ArrowLeft, Send, Shield, MapPin, AlertCircle, Clock, Flame, X, Lock, Crown } from 'lucide-react'
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center h-screen gap-4 p-6">
        ...
        <Link href="/matches" className="bg-gold-500 hover:bg-gold-400 text-dark-900 font-bold px-6 py-2.5 rounded-full text-sm transition-colors">
          Voltar para Matches
        </Link>
      </div>
    )
  ...
  return (
    <div className="relative flex flex-col h-screen pb-16 md:pb-0">
      ...
      <div className="bg-dark-800 border-b border-dark-600 px-4 py-3 flex items-center gap-3 shrink-0">
        <Link href="/matches" className="text-dark-300 hover:text-white transition-colors p-1">
          <ArrowLeft size={20} />
        </Link>
        {otherUser && (
          <>
          ...
          return (
            <div key={msg.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-xs lg:max-w-md px-4 py-2.5 rounded-2xl text-sm ${isMine
                ? 'bg-gold-500/20 border border-gold-500/20 text-white rounded-br-sm'
                : 'bg-dark-700 border border-dark-600 text-white rounded-bl-sm'
              }`}>
                <p>{msg.content}</p>
        ...
        {canSend ? (
          <form onSubmit={sendMessage} className="flex items-center gap-3">
            <input
              ...
              className="flex-1 bg-dark-700 border border-dark-500 rounded-xl px-4 py-2.5 text-sm text-white placeholder-dark-400 focus:outline-none focus:border-gold-500/50 transition-colors"
            />
            <button
              type="submit"
              disabled={!input.trim() || sending}
              className="w-10 h-10 rounded-xl bg-gold-500 hover:bg-gold-400 disabled:opacity-50 text-dark-900 flex items-center justify-center transition-colors shrink-0"
            >
              {sending
          ...
          <div className="glass rounded-2xl p-6 border border-pink-500/20 w-full max-w-sm">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold">Solicitar Foto Hot</h3>
              </div>
              ...
              <>
                ...
                <div className="mb-4">
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full bg-dark-800 border border-dark-500 rounded-xl px-4 py-2.5 text-sm text-white placeholder-dark-400 focus:outline-none focus:border-pink-500/40 transition-colors resize-none"
                  />
                </div>
