...
import {
  ...
  AlertCircle, Camera, X, Flame, Grid3X3, ChevronDown, ChevronUp
} from 'lucide-react'
  ...
  if (error || !profile) {
    return (
      <div className="flex flex-col items-center justify-center h-screen gap-4 p-6">
        ...
        <Link href="/discover" className="bg-gold-500 hover:bg-gold-400 text-dark-900 font-bold px-6 py-2.5 rounded-full text-sm transition-colors">
          Voltar
        </Link>
      </div>
    )
  ...
  return (
    <div className="min-h-screen pb-24 md:pb-8">
      ...
      <div className="sticky top-0 z-30 bg-dark-800/90 backdrop-blur-lg border-b border-dark-700/60 px-4 py-3 flex items-center gap-3">
        <Link href="/discover" className="text-dark-300 hover:text-white transition-colors p-1">
          <ArrowLeft size={20} />
        </Link>
          ...
          <div className="absolute right-4 bottom-3 flex items-center gap-2">
            ...
            {matchId ? (
              <Link
                ...
                className="flex items-center gap-1.5 bg-gold-500 hover:bg-gold-400 text-dark-900 font-bold px-4 py-2 rounded-full text-sm transition-colors"
              >
              ...
              <button
                onClick={handleLike}
                disabled={liked}
                className={`flex items-center gap-1.5 font-bold px-4 py-2 rounded-full text-sm transition-all ${liked
                  ? 'bg-gold-500/20 border border-gold-500/40 text-gold-400'
                  : 'bg-gold-500 hover:bg-gold-400 text-dark-900'
                }`}
              >
                <Heart size={15} className={liked ? 'fill-gold-400' : ''} />
      ...
      <div className="px-5 pt-16 pb-4">
        ...
        {!isOwnProfile && isMaleViewing && profile.role === 'FEMALE' && (
          <button
            ...
            className="mt-4 w-full py-3 rounded-xl font-bold flex items-center justify-center gap-2 bg-dark-700/60 border border-pink-500/30 text-pink-400 hover:bg-pink-500/10 hover:border-pink-500/50 transition-all text-sm"
          >
            <Flame size={16} />
            ...
            <div className="px-4 pb-4 space-y-3 border-t border-dark-700/50 pt-3">
              ...
              {profile.sugarProfile.allowanceRange && (
                <div className="flex justify-between items-center">
                  ...
                  <p className="text-sm font-semibold text-white">{profile.sugarProfile.allowanceRange}</p>
                </div>
              )}
              {profile.sugarProfile.meetingFrequency && (
                <div className="flex justify-between items-center">
                  <p className="text-xs text-dark-400">Frequência de encontros</p>
                  <p className="text-sm font-semibold text-white">{profile.sugarProfile.meetingFrequency}</p>
                </div>
              )}
              {profile.sugarProfile.availability && (
                <div className="flex justify-between items-center">
                  <p className="text-xs text-dark-400">Disponibilidade</p>
                  <p className="text-sm font-semibold text-white">{profile.sugarProfile.availability}</p>
                </div>
              )}
              ...
              >
                ...
                {i === 0 && profile.verified && (
                  <span className="absolute top-1.5 left-1.5 bg-gold-500/90 rounded-full p-1">
                    <Shield size={9} className="text-dark-900" />
                  </span>
                )}
              </button>
            ))}
        ...
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="glass rounded-2xl p-6 border border-pink-500/20 w-full max-w-sm">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowHotModal(false)} className="text-dark-300 hover:text-white">
                <X size={20} />
              </button>
            </div>
              ...
              <>
                ...
                <div className="mb-4">
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full bg-dark-700 border border-dark-600 rounded-xl px-4 py-2.5 text-sm text-white placeholder-dark-400 focus:outline-none focus:border-pink-500/40 transition-colors resize-none"
                  />
                </div>
                ...
                <button
                  ...
                  disabled={hotLoading}
                  className="w-full py-3 rounded-xl font-bold bg-pink-500 hover:bg-pink-400 text-white disabled:opacity-50 transition-colors flex items-center justify-center gap-2"
                >
                  {hotLoading
