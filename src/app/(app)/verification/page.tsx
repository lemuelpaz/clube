    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6">
        <div className="glass rounded-2xl p-10 border border-red-500/20 text-center max-w-md">
          ...
          <button onClick={() => setVerif(null)} className="bg-gold-500 hover:bg-gold-400 text-dark-900 font-bold px-6 py-3 rounded-full text-sm transition-colors">
            Tentar Novamente
          </button>
        </div>
      </div>
          ...
          <div className="flex items-center gap-2 mb-8">
            {[1, 2, 3].map(s => (
              <div key={s} className="flex items-center gap-2">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all ${step >= s ? 'bg-gold-500 text-dark-900' : 'bg-dark-700 text-dark-400'}`}>
                  {step > s ? '✓' : s}
                </div>
            ...
            <div className="space-y-4">
              ...
              <div className="border-2 border-dashed border-dark-500 rounded-xl p-8 text-center">
                ...
                <input type="text" placeholder="URL da imagem do documento (demo)"
                  ...
                  className="w-full bg-dark-800 border border-dark-500 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-gold-500/50" />
              </div>
              <button onClick={() => docUrl && setStep(2)} disabled={!docUrl}
                className="w-full bg-gold-500 hover:bg-gold-400 disabled:opacity-40 text-dark-900 font-bold py-3 rounded-xl transition-colors">
                Continuar
              </button>
            </div>
          )}
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={() => selfieUrl && setStep(3)} disabled={!selfieUrl}
                  className="flex-1 bg-gold-500 hover:bg-gold-400 disabled:opacity-40 text-dark-900 font-bold py-3 rounded-xl transition-colors">
                  Continuar
                </button>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={handleSubmit} disabled={submitting}
                  className="flex-1 bg-gold-500 hover:bg-gold-400 disabled:opacity-40 text-dark-900 font-bold py-3 rounded-xl transition-colors flex items-center justify-center gap-2">
