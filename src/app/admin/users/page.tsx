        ...
        <div className="flex gap-3 flex-wrap">
          <div className="relative flex-1 min-w-48">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)}
              ...
              className="w-full bg-dark-800 border border-dark-600 rounded-xl pl-9 pr-4 py-2.5 text-sm text-white placeholder-dark-400 focus:outline-none focus:border-gold-500/50" />
          </div>
          {(['ALL', 'MALE', 'FEMALE'] as const).map(r => (
            <button key={r} onClick={() => setRoleFilter(r)}
              className={`px-4 py-2.5 rounded-xl text-sm font-medium transition-colors ${roleFilter === r ? 'bg-gold-500/20 text-gold-400 border border-gold-500/30' : 'bg-dark-800 border border-dark-600 text-dark-300 hover:text-white'}`}>
