          ...
          <div className="flex gap-2">
            {(['ALL', 'PENDING', 'APPROVED', 'REJECTED'] as const).map(f => (
              <button key={f} onClick={() => setFilter(f)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${filter === f ? 'bg-gold-500/20 text-gold-400 border border-gold-500/30' : 'text-dark-300 hover:text-dark-50'}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="glass rounded-2xl p-6 border border-red-500/20 w-full max-w-md">
            ...
            <textarea value={rejectReason} onChange={e => setRejectReason(e.target.value)}
              ...
              className="w-full bg-dark-800 border border-dark-500 rounded-xl px-4 py-3 text-sm text-dark-50 focus:outline-none focus:border-red-500/50 resize-none mb-4" />
            <div className="flex gap-3">
              ...
              <button onClick={() => reject(rejectModal)}
                className="flex-1 bg-red-500/80 hover:bg-red-500 text-dark-50 py-2.5 rounded-xl text-sm font-bold transition-colors">
                Rejeitar
              </button>
            </div>
          </div>
