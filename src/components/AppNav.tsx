    ...
    <>
      ...
      <aside className="hidden md:flex fixed left-0 top-0 h-full w-64 bg-dark-800 border-r border-dark-600 flex-col z-40">
          ...
          {SIDEBAR_NAV.map(({ href, icon: Icon, label }) => {
            ...
            return (
              <Link key={href} href={href}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all ${active ? 'bg-gold-500/15 text-gold-400 font-medium' : 'text-dark-200 hover:text-white hover:bg-dark-700'}`}>
                <Icon size={18} />
                {label}
              </Link>
            )
          ...
          {user.role === 'FEMALE' && (
            <>
              <Link href="/verification"
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all ${path.startsWith('/verification') ? 'bg-gold-500/15 text-gold-400 font-medium' : 'text-dark-200 hover:text-white hover:bg-dark-700'}`}>
                <Shield size={18} />
                Verificação
              </Link>
              <Link href="/earnings"
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all ${path.startsWith('/earnings') ? 'bg-gold-500/15 text-gold-400 font-medium' : 'text-dark-200 hover:text-white hover:bg-dark-700'}`}>
                <Wallet size={18} />
                Ganhos
              </Link>
            </>
        ...
        <div className="p-3 border-t border-dark-600 space-y-1">
          <Link href="/profile"
            className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-dark-200 hover:text-white hover:bg-dark-700 transition-all">
            <Settings size={18} />
            Configurações
          </Link>
